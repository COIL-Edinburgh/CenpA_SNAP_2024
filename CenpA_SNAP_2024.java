...
public class CenpA_SNAP_2024 implements PlugIn {
    ...
    public void run(String arg) {
        ...
        String newDirectory = baseDirectory + "Output_v2";
        filePath = Paths.get(baseDirectory, "Output_v2").toString();
...
//
                    ...
                    roiManager.runCommand("Save", Paths.get(filePath, filename + "_v2.zip").toString());
                    outputData(data);

                    compImage = drawCells(compImage, cellRois, rois);
                    IJ.saveAs("Tiff", Paths.get(filePath, filename + "_v2.tif").toString());
                    IJ.log(Paths.get(filePath, filename + "_v2.tif").toString());
                }else {
    ...
    private double[][] getData(Roi[] rois, ImagePlus dataZproj, Roi[] cellRois) {
        roiManager.reset();
        double corner = (squareSize - 1) / 2;
        int count = 1;
        double[][] data= new double[3][rois.length];
        ...
        try{
            ...
            for (int i =0; i< data[0].length; i++){
                if(data[0][i]!=0 && data[2][i]!=0){
                    bufferedWriter.write(data[0][i]+","+data[1][i]+","+data[2][i]);
                    bufferedWriter.newLine();
                }}
            bufferedWriter.close();
        }
